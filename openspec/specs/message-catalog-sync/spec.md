...
The system SHALL support message extraction from the package manifest domain, from supported webview sources, and from supported runtime dynamic formatter and QuickPick sources in the current phase. Its catalog model SHALL preserve a domain-adapter boundary so later runtime domains and unsupported source kinds can be added without redesigning the catalog structure.
...
#### Scenario: Runtime dynamic formatter or QuickPick message is extracted

- **WHEN** a supported formatter or QuickPick low-level source pattern is scanned
- **THEN** the runtime dynamic adapter emits occurrences using source-code references and runtime-oriented output references
- **AND** the occurrence records the domain and scope needed to distinguish formatter text from QuickPick text

#### Scenario: Future or unsupported runtime source is deferred without redesigning the model

- **WHEN** a runtime dynamic source kind is not yet implemented in the current phase
- **THEN** the catalog model still retains domain and anchor abstractions needed to add that source later without changing authority or override semantics
- **AND** the workflow reports the source as deferred rather than silently treating it as localized
